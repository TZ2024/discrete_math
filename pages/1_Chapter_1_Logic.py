...
def render_overview():
    ...
    st.markdown("""
    This app implements the "Bridge" concept discussed in our meetings:
    
    1.  **Sets to SQL (6.1, 6.10)**: Relations are Tables (Selection/Projection).
    2.  **Modeling (6.3, 6.6)**: Graphs are Relations on a single set ($V \\times V$).
    3.  **Operations (6.4, 6.5)**: Matrix Multiplication calculates "Reachability".
    4.  **Applications (6.7 - 6.9)**: Algorithms for Scheduling and Clustering.
    """)
    st.info("👈 Use the tabs above to explore.")

# --- Tab 1: Basics (Bridge to DB) ---
def render_basics():
    st.subheader("1. The Bridge: Sets ↔ Tables")
    st.markdown("General Binary Relations ($A \\times B$).")
    
    with st.expander("🛠️ Define Relation (Two Sets)", expanded=True):
    ...
    if A and B:
        ...
        with c_math:
            ...
            # 只有当 A=B 时才显示属性，否则没有意义
            if A == B:
                props = check_properties(A, rel)
                st.caption(f"Properties on Set {A}:")
                st.json(props)
            else:
                st.caption("(Properties like Reflexive only apply when Set A = Set B)")
...
def render_modeling():
    st.subheader("2. Modeling: Digraphs & Matrix Powers")
    st.markdown("Connecting **Section 6.3** (Graphs) & **Section 6.6** (Matrix Ops).")
    
    # 【核心修改】强制使用单个集合 V，符合 digraph 定义
    with st.expander("🕸️ Define Graph Nodes (Set V)", expanded=True):
        c1, c2 = st.columns([1, 2])
        nodes = parse_set_input(c1.text_input("Vertices (V)", "1, 2, 3, 4"))
        rule = c2.selectbox("Edge Rule (V × V)", [
            "Divides (a | b)", 
            "Less Than (a < b)",
            "Greater Than (a > b)"
        ])
    ...
    with col2:
        st.markdown("#### 🔢 Adjacency Matrix ($M$)")
    ...
    # 矩阵幂演示 (Social Network Reachability)
    st.markdown("#### 🚀 Matrix Powers ($M^k$) - Reachability")
    st.write("Dr. Murali mentioned: $M^2$ represents 'Friends of Friends' (Path length 2).")
    
    k = st.slider("Path Length (k)", 1, 4, 2)
    m_pow = matrix_power(matrix, k)
    
    st.write(f"**Matrix $M^{k}$:** Shows connectivity in exactly {k} steps.")
...
def render_operations():
    ...
    with tab1:
        ...
        st.markdown("This demonstrates Selection and Projection (No Join, as per textbook scope).")
...
def render_applications():
    ...
    with tab_sched:
        ...
        st.info("Algorithm: Find node with in-degree 0 -> Remove it -> Repeat.")
